...
contract Verifier is BaseVerifier {
    ...
    ///         2. The oldKey can decrypt the pre-image and the new key re-encrypt the plaintext to new ciphertext
    ///         3. The newKey is encrypted with the receiver's pubKey to get the sealedKey
    ///         4. The hashes of new ciphertext is newDataHashes (key to note: TEE could support a private key of the receiver)
    ///         5. The newDataHashes identified ciphertext are available in the storage: need the signature from the receiver signing oldDataHashes and newDataHashes
    /// @param proofs Proof generated by TEE
